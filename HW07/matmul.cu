...
__global__ void Muld_float(float* A, float* B, float* C, unsigned int n, unsigned int block_dim)
{
    ...
    float* As=shared;
    float* Bs=shared + block_dim*block_dim;
...
__global__ void Muld_int(int* A, int* B, int* C, unsigned int n, unsigned int block_dim)
{
    ...
    int ty = threadIdx.y; //the row index in the sub-block
    int* As=shared;
    int* Bs=shared + block_dim*block_dim;
    int row = by * block_dim + ty;
    int col = bx * block_dim + tx;
...
__global__ void Muld_doub(double* A, double* B, double* C, unsigned int n, unsigned int block_dim)
{
    ...
    int ty = threadIdx.y; //the row index in the sub-block
    double* As=shared;
    double* Bs=shared + block_dim*block_dim;
    int row = by * block_dim + ty;
    int col = bx * block_dim + tx;
